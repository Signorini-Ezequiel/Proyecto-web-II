    ...
    <main class="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 pt-20">
      <section class="border-b border-slate-200 bg-white py-8">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p class="text-slate-600">Selecciona entre 2 y 4 vehículos guardados para una comparativa detallada.</p>
        </div>
      </section>
        ...
        <div class="rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-sm">
          <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p class="mt-2 text-sm text-slate-600">Los autos publicados también se pueden comparar aunque tengan especificaciones faltantes.</p>
            </div>
            <button
              type="button"
            ...
            favoriteCars.length === 0
              ? `
                <div class="mt-6 rounded-2xl border border-[#e76e1d]/15 bg-[#fff4eb] p-8 text-center">
                  <p class="text-base font-semibold text-slate-900">No tienes autos guardados aún.</p>
                  <p class="mt-2 text-sm text-slate-600">Agrega favoritos desde Buscar y luego podrás compararlos acá.</p>
                </div>
              `
              : `
                <div class="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  ${favoriteCars
                    .map((car) => {
                      ...
                      return `
                        <label class="relative flex cursor-pointer items-start gap-3 rounded-2xl border px-4 py-4 transition ${
                          isSelected
                          ...
                          <span class="min-w-0 flex-1">
                            <span class="block text-sm font-semibold ${isSelected ? "text-[#9a3f05]" : "text-slate-900"}">${car.make} ${car.model}</span>
                            <span class="mt-1 block text-sm text-slate-600">${car.year} · ${car.transmission} · ${car.fuel}</span>
                            <span class="mt-2 block text-xs uppercase tracking-[0.22em] text-[#c9540a]">US$ ${car.price.toLocaleString()}</span>
                          </span>
                        </label>
                      `;
                    })
      ...
      ${
        selectedCars.length < 2
          ? `
            <section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
              <div class="rounded-3xl border border-[#e76e1d]/15 bg-[#fff4eb] p-8 text-center">
                <p class="text-lg font-semibold text-slate-900">Selecciona al menos 2 vehículos para ver la comparación.</p>
          ...
          : `
            <section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
              <h2 class="mb-6 text-2xl font-bold text-slate-900">Vehículos Seleccionados</h2>
            ...
            <section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
              <h2 class="mb-6 text-2xl font-bold text-slate-900">Análisis Inteligente</h2>
            ...
            <section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
              ...
              <div class="mb-4 rounded-2xl border border-[#e76e1d]/15 bg-[#fff4eb] p-4">
                <p class="text-sm text-slate-700"><strong>Nota:</strong> Las celdas destacadas muestran la mejor opción en cada categoría.</p>
              </div>
              ${ComparisonTable({ cars: selectedCars, metrics: metrics! })}
            </section>

            <section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
              <h2 class="mb-6 text-2xl font-bold text-slate-900">Desglose de Puntuación</h2>
